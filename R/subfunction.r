...
iNEXTPDlink = function (data, datatype = "abundance", col.tree = NULL, row.tree = NULL, q = c(0,
                                                                                              1, 2), reftime = NULL, type = "meanPD", endpoint = NULL,
                        knots = 40, size = NULL, nboot = 50, conf = 0.95)
{
  ...
  index <- rbind(Asy.link(data = data, diversity = "PD",row.tree = row.tree, col.tree  = col.tree,
                          q = c(0, 1, 2), datatype = datatype, PDtype = type, nboot = nboot,
                          conf = 0.95), Obs.link(data = data,  diversity = "PD",row.tree = row.tree, col.tree  = col.tree,
                                                 q = c(0, 1, 2), datatype = datatype, PDtype = type, nboot = nboot,
                                                 conf = 0.95))
  ...
  if (datatype == "abundance") {
    ...
    Estoutput <- lapply(1:length(datalist), function(i) {
      ...
      x <- datalist[[i]] %>% .[rowSums(.) > 0, colSums(.) >
                                 0]
      n <- ncol(x)
      qPDm <- PhD.m.est(ai = aL$treeNabu$branch.abun,
      ...
      if (nboot > 1) {
        ...
        if (unconditional_var) {
          ses <- sapply(1:nboot, function(B) {
            ...
            isn0 <- ai_B > 0
            qPDm_b <- PhD.m.est(ai = ai_B[isn0], Lis = Li_b[isn0,
                                                            , drop = F], m = m[[i]], q = q, nt = n,
                                reft = reft, cal = cal) %>% as.numeric()
                        ...
                        threshold = NULL, nT = NULL)
{
  ...
  index <- rbind(iNEXT.3D:::AsyFD(data = data1, distM = distM, q = c(0,
                                                                     1, 2), datatype = datatype, nboot = nboot, conf = 0.95,
                                  threshold = NULL), iNEXT.3D:::ObsFD(data = data1, distM = distM,
                                                                      q = c(0, 1, 2), datatype = datatype, nboot = nboot,
                                                                      conf = 0.95, threshold = NULL))
                         ...
                         threshold = NULL, nT = NULL)
{
  ...
  index <- rbind(iNEXT.3D:::AsyAUC(data = data1, distM = distM, q = c(0,
                                                                      1, 2), datatype = datatype, nboot = nboot, conf = 0.95),
                 iNEXT.3D:::ObsAUC(data = data1, distM = distM, q = c(0, 1, 2), datatype = datatype,
                                   nboot = nboot, conf = 0.95))
